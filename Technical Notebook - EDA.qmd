...
# -----------------------------------------------------------------------------
# Clean date/time and recode into major felony categories according to the
# CompStat 7 major felony categories
# -----------------------------------------------------------------------------
brooklyn_clean <- brooklyn_raw |>
  mutate(
    # ensure class is Date
    ...
    # CompStat 7 major felonies from NYPD PD descriptions
   felony_category = case_when(
      # 1) Murder & Non-Negligent Manslaughter
      str_detect(pd_desc, regex("MURDER|HOMICIDE|MANSLAUGHTER", ignore_case = TRUE)) ~
        "Murder & Non-Negligent Manslaughter",

      # 2) Rape
      str_detect(
        pd_desc,
        regex("RAPE|SEXUAL ASSAULT|SEX ABUSE|SEXUAL ABUSE|SEXUAL MISCONDUCT|SEX CRIME",
              ignore_case = TRUE)
      ) ~ "Rape",

      # 3) Robbery
      str_detect(pd_desc, regex("\\bROBBERY\\b", ignore_case = TRUE)) ~ "Robbery",

      # 4) Felony Assault
      str_detect(
        pd_desc,
        regex("ASSAULT|STRANGULATION|AGGRAVATED HARASSMENT", ignore_case = TRUE)
      ) ~ "Felony Assault",

      # 5) Burglary
      str_detect(pd_desc, regex("BURGLARY", ignore_case = TRUE)) ~ "Burglary",

     # 6) Grand Larceny of Motor Vehicle - FIXED PATTERN!
      str_detect(
        pd_desc,
        regex("LARCENY,GRAND OF AUTO|LARCENY,GRAND.*VEHICLE|LARCENY,GRAND.*MOTORCYCLE|UNAUTHORIZED USE VEHICLE|STOLEN PROPERTY.*MOTOR",
              ignore_case = TRUE)
      ) ~ "Grand Larceny of Motor Vehicle",

      # 7) Grand Larceny (non-auto)
      str_detect(pd_desc, regex("LARCENY,GRAND", ignore_case = TRUE)) ~ "Grand Larceny",
  ...
  select(felony_date, felony_hour, pd_cd, pd_desc, felony_category)
write_csv(brooklyn_clean, "brooklyn_felonies_clean_records.csv")
...
# -----------------------------------------------------------------------------
# Create CSV and tsibble of the HOURLY total felony counts
# -----------------------------------------------------------------------------
hourly_total <- brooklyn_clean |>
  mutate(
    felony_datetime = ymd_h(paste(felony_date, felony_hour))
  ) |>
  filter(!is.na(felony_datetime)) |>
  count(felony_datetime, name = "felony_count") |>
  arrange(felony_datetime)

write_csv(hourly_total, "hourly_total_felonies.csv")

hourly_total_ts <- hourly_total |>
  as_tsibble(index = felony_datetime)

# -----------------------------------------------------------------------------
# Create CSV and tsibble of the DAILY felony counts by CompStat7 category
# -----------------------------------------------------------------------------
daily_by_cat <- brooklyn_clean |>
  count(felony_date, felony_category, name = "felony_count") |>
  arrange(felony_date, felony_category)

write_csv(daily_by_cat, "daily_felonies_by_category.csv")

daily_by_cat_ts <- daily_by_cat |>
  as_tsibble(index = felony_date, key = felony_category)

# -----------------------------------------------------------------------------
# Create CSV and tsibble of the HOURLY felony counts by CompStat7 category
# -----------------------------------------------------------------------------
hourly_by_cat <- brooklyn_clean |>
  mutate(
    felony_datetime = ymd_h(paste(felony_date, felony_hour))
  ) |>
  filter(!is.na(felony_datetime)) |>
  count(felony_datetime, felony_category, name = "felony_count") |>
  arrange(felony_datetime, felony_category)

write_csv(hourly_by_cat, "hourly_felonies_by_category.csv")

hourly_by_cat_ts <- hourly_by_cat |>
  as_tsibble(index = felony_datetime, key = felony_category)
```
```{r}
# Check categories
brooklyn_clean |>
  count(felony_category) |>
  arrange(desc(n))
```
...
# -----------------------------------------------------------------------------
# Looking at ACF/PACF plots to determine if differencing is necessary
#------------------------------------------------------------------------------

# -----------------------------------------------------------------------------
# ACF and PACF for DAILY total felonies
# -----------------------------------------------------------------------------
daily_acf <- daily_total_ts |>
  ACF(felony_count, lag_max = 60) |>
  autoplot() + 
...
# -----------------------------------------------------------------------------
# ACF and PACF for DAILY by category (faceted)
# First fill in the gaps with zeros for missing date/category combinations
# -----------------------------------------------------------------------------
daily_by_cat_ts <- daily_by_cat_ts |>
  fill_gaps(felony_count = 0)

daily_cat_acf <- daily_by_cat_ts |>
  ACF(felony_count, lag_max = 60) |>
  autoplot() + 
  facet_wrap(~ felony_category, scales = "free_y") +
  ggtitle("Daily by Category - ACF")

daily_cat_pacf <- daily_by_cat_ts |>
  PACF(felony_count, lag_max = 60) |>
  autoplot() + 
  facet_wrap(~ felony_category, scales = "free_y") +
  ggtitle("Daily by Category - PACF")

daily_cat_acf
daily_cat_pacf

# -----------------------------------------------------------------------------
# ACF and PACF for HOURLY by category (faceted)
# -----------------------------------------------------------------------------
hourly_by_cat_ts <- hourly_by_cat_ts |>
  fill_gaps(felony_count = 0)

hourly_cat_acf <- hourly_by_cat_ts|>
  ACF(felony_count, lag_max = 168) |>
  autoplot() + 
  facet_wrap(~ felony_category, scales = "free_y") +
  ggtitle("Hourly by Category - ACF")

hourly_cat_pacf <- hourly_by_cat_ts |>
  PACF(felony_count, lag_max = 168) |>
  autoplot() + 
  facet_wrap(~ felony_category, scales = "free_y") +
  ggtitle("Hourly by Category - PACF")
