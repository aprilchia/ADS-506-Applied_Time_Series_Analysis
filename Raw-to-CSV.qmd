...
brooklyn_clean <- brooklyn_raw |>
  mutate(
    ...
    felony_category = case_when(
      ...
      # 2) Rape (include sexual assault / sex abuse)
      str_detect(
        pd_desc,
      ...
      # 4) Felony Assault (including strangulation, aggravated assault, etc.)
      str_detect(
        pd_desc,
      ...
      # 6) Grand Larceny of Motor Vehicle (GLA / auto theft)
      str_detect(
        pd_desc,
        regex("GRAND LARCENY AUTO|GLA|MOTOR VEHICLE THEFT|AUTO STRIPPING",
              ignore_case = TRUE)
      ) ~ "Grand Larceny of Motor Vehicle",

      # 7) Grand Larceny (non-auto)
      str_detect(pd_desc, regex("GRAND LARCENY", ignore_case = TRUE)) ~ "Grand Larceny",

      # Everything else: still a felony, but not one of the CompStat 7
      TRUE ~ "Other Felony"
    )
  ) |>
  filter(
